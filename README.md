...
GitHubからクローンしたプロジェクトが正しく動作するか確認するために、
ローカル環境・Dockerコンテナ・データベースを一度削除してクリーンな状態から再構築します。

### 1. Dockerコンテナとボリュームの削除
まず、実行中のコンテナとデータベースのボリュームを削除します。

```bash
docker-compose down -v
```

### 2. ローカルプロジェクトディレクトリの削除
一つ上の階層に移動してから、対象ディレクトリを削除します。

```bash
cd ..
rm -rf weight-logs-test3
```

### Permission denied が出る場合はこちらを使用
```bash
sudo rm -rf weight-logs-test3
```
